...
main(){
	...
	echo $SERVICE
		...
		if [ "$feature" == "getAPI" ] 
		then
			echo "$PDIR/$SERVICE/TestSuite/$feature.sh $CUSTOMER.properties" >> runFeatures.$SERVICE.$CUSTOMER.sh
...
echo "getReleaseVersionInfo $SERVICE $CUSTOMER" >> runFeatures.$SERVICE.$CUSTOMER.sh
...
#	mv $PDIR/$1_*.txt $PDIR/$serviceName/Result/
	...
	mv $PDIR/INC/*.log $PDIR/$serviceName/Log/$dirName/
...
#	tcpdump -i any -w $filename &
	...
	if [ "x$CAS_ip" != "x" ]
        ...
        echo "going to use below command: tcpdump -i any -w $filename $HOST "
         tcpdump -i any -w $filename $HOST &
...
checkCDR()
{
	...
	incFlag=`echo $1| egrep "inc|ht_"`
	if [ "x$incFlag" == "x" ]
	then
		./Utility/TestRexec -host $CAS_ip -userId $CAS_user -passwd $CAS_pass -cmd  grep $Local_Host $CDRPath$dateFolder/* > AutomationCallsCDR
	else 	./Utility/TestRexec -host $CAS_ip -userId $CAS_user -passwd $CAS_pass -cmd  grep -v $Local_Host $CDRPath$dateFolder/* > AutomationCallsCDR
	fi
...
checkHTTPInfo()
{
	...
	./Utility/compResult.sh $httpInfoFile SIPP_XML_Result.txt $1 
...
getReleaseVersionInfo() {
	...
	versionFile="$PDIR/$serviceName/ReleaseVersionInfo.$serviceName.$customerName"
echo " ./Utility/TestRexec -host $CAS_ip -userId $CAS_user -passwd $CAS_pass -cmd  cat $versionFileInfo > $versionFile"
	./Utility/TestRexec -host $CAS_ip -userId $CAS_user -passwd $CAS_pass -cmd  cat $versionFileInfo > $versionFile
	grep -v "#" $versionFile | grep -v "rexec" |while read -r var1
	do
	        service=`echo $var1 |cut -d ":" -f1`
		version=`echo $var1 |cut -d ":" -f2`
		detail=`echo $var1 |cut -d ":" -f3`

		echo "<div style=display:none ><Version><ServiceName>$service</ServiceName><VID>$version</VID><Description>$detail</Description><Result>PASS</Result><Steps><Step id=0 status=Info>Release Version info</Step></Steps></Version></div>" >> $PDIR/$1/$1.$2.html
	done
}

getBugID() {

        var1=`grep "$1" $bugFileID | cut -d ":" -f2`
	return $var1
