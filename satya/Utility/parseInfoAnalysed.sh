...
getBuffer(){
	...
	egrep -v "^#|^[ ]*$" $tcID.XML | while read LINE ; do
		...
		callID_Tag=`echo $LINE| grep "sip.Call-ID" | grep "ASE_" | grep "satya"`
		t1="${callID_Tag#*satya-}"
		t2=`echo $t1|cut -d "/" -f1`	
		if [ "x$t2" != "x" ] 
		then
...
#			notify_callID="INC_$notify_callID"
			continue
		fi
...
parseInfoAnalysed(){	
	...
	export IA_Data_File="$PDIR/$serviceName/Data/IA_$1.csv"
...
export IA_Results="$PDIR/$serviceName/$RESULTS/IA_Results_$1.csv"
	...
	if [ ! -e "$PDIR/$serviceName/$tcID.pcap" ]
	 then
		...
		tshark -n -r $PDIR/$serviceName/$tcID.pcap  -T pdml > tmp.xml
		egrep "sip.msg_body|ASE_|0d0a" tmp.xml > $tcID.XML
		getBuffer
	fi
